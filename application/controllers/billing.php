...
class Billing extends CI_Controller 
{
	...
	public function index() {
		...
		redirect(base_url() . "billing/searchByName");
	}
...
// - - - - -  - - - - - - - - - - - - - - - -  -gui part for packages - - - - -  - - - - - - - - - - - - - - - - - - - - -  - - - - - - - - - - - - - - - - - -
		$visit_num = $this->queryDatabase("visit_num", "appointments", "true_id = '$id'")[0];
		$order_ids = $this->queryDatabase("order_id", "order_list", "visit_num = '$visit_num' AND order_id NOT IN (SELECT order_id FROM `packages_per_bill`)");
		$products = "";
			...
			for($i = 0; $i < count($package['name']); $i++) {
				if($package['type'][$i] == "P") {
					$products = $products . "<tr>" .
												...
												"<td><center>" . $package['case_rate'][$i] . "</center></td>" .
		...
		$an_cost = array_sum($this->queryDatabase("an_cost", "additional_costs", "$visit_num = visit_num AND anreq_id NOT IN (SELECT anreq_id FROM anesthesias_per_bill)"));
		$other_cost = array_sum($this->queryDatabase("other_cost", "additional_costs", "$visit_num = visit_num AND anreq_id NOT IN (SELECT anreq_id FROM anesthesias_per_bill)"));
...
// - - - - - - - -  - - - - - - - - gui part for recordables  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 	
	...
	public function recordNewBill() {
		...
		$anreq_ids = $this->queryDatabase("anreq_id", "additional_costs", "$visit_num = visit_num AND anreq_id NOT IN (SELECT anreq_id FROM anesthesias_per_bill)");
...
// --- previous transactions
	...
	public function generateInvoice($newdata, $newdata2){
		...
		$pdf->Output($lastname.'_Invoice.pdf', 'D');
	}
	...
	public function generateAck($newdata, $newdata2){
		...
		$html = '<p align = "center"><h4> Alvina Pauline D. Santiago, M.D.</h4><br/>
		 Computerised Clinic System for Opthalmology <br/>
		 Padre Faura St. Ermita, Manila  <br/>
		 Telephone # 524-30-11</p> <hr>';
		$pdf->writeHTML($html, true, false, true, false, '');
		...
		$pdf->writeHTML($html, true, false, true, false, '');
		$pdf->writeHTMLCell(0,0,'',100,'<b>Amount: </b>',0, 0, 0, true, '', true);
		...
		$pdf->Output($lastname.'_AckSlip.pdf', 'D');
